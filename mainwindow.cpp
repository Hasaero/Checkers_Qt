...
void MainWindow::on_StartButton_cpu_pressed()
{
...
void MainWindow::on_button_clicked(){
    ...
    if (isLiftTurn) { // When user lifts a piece, do 'pickPiece' movement...
        try {
            ...
            isLiftTurn = !isLiftTurn; // It's time to put down a piece
            return;
          // If the 'pickPiece' funtion throws an exception, show the exception message.
            ...
            } // After player put down a piece, it's time to CPU
                if (isComputer){
                    string pickedButtonName_CPU;
                    // When CPU picks up the piece
                    for (auto iter = map_s_Q.rbegin(); iter != map_s_Q.rend(); iter++){
                      pickedButtonName_CPU = iter->first;
                      QLabel* pickedLabelPtr_CPU = map_s_Q.at(pickedButtonName_CPU);
                      try {
                          Move::pickPiece(pickedButtonName_CPU, pickedLabelPtr_CPU);// @assign: moveFromButtonName, movingLabelPtr
                          break;
                      }
                      catch (invalid_argument& e) {
                          continue;
                    ...
                    // When CPU put down the piece
                    isLiftTurn = !isLiftTurn;
                    for (auto iter = map_s_Q.rbegin(); iter != map_s_Q.rend(); iter++){
                      pickedButtonName_CPU = iter->first;
                      QLabel* pickedLabelPtr_CPU = map_s_Q.at(pickedButtonName_CPU);
                      // 모든 버튼을 아래에서부터 하나씩 눌러본다.
                      if (pickedLabelPtr_CPU != nullptr) continue; // Other pieces already exists -> skip!
                      try {
                        Move::movePiece(movingLabelPtr, moveFromButtonName, pickedButtonName_CPU);
                        moveToButtonName = pickedButtonName;
                        handleKingPiece(movingLabelPtr, moveToButtonName); // if movePiece is done, check if the moved piece is King and handle it.
                        break;
                      } catch (invalid_argument& e) {
